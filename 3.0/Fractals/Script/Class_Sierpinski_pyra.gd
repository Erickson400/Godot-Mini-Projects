...
var mainMat_mat = preload("res://Assets/Red.tres")
...
func Iterate(Size:float):
	...
	for i in Pyra_2.size():
		Pyra_2[i] += Vector3(0, 0, -Size)
	for i in Pyra_3.size():
		Pyra_3[i] += Vector3(Size, 0, -Size)
	for i in Pyra_4.size():
		Pyra_4[i] += Vector3(Size/2, Size, -Size/2)
...
func Func_vertex_thread(vertex_thread_userdata):
	...
	vertices.append_array(vertex_thread_userdata[3])
...
func Make_Pyramid(Offset:Vector3, Size):
	Make_Triangle(4, 1, 0, Offset, Size)
	Make_Triangle(4, 2, 1, Offset, Size)
	Make_Triangle(4, 3, 2, Offset, Size)
	Make_Triangle(4, 0, 3, Offset, Size)
	Make_Triangle(0, 1, 2, Offset, Size)
	Make_Triangle(2, 3, 0, Offset, Size)
